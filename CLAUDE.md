...
- Commit message format: `type(scope): description [agent]`
...
- Branch naming: `agent/<short-description>`
- Never commit directly to `main`, `master`, `develop`, or `release/*`
- PR target: `develop`
- Always open PRs as **Draft**
- Keep one task per branch and one PR per branch
...
- After completing a research phase, write a short summary before proceeding.

---

## Claude Workspace Files

- `CLAUDE.md` is the primary instruction file Claude reads automatically.
- `.claude/claude_config.json` defines Claude-specific MCP connections.
- `.claude/agents/` contains reusable sub-agent or role definitions.
- `.claude/hooks/` contains Claude-specific hook scripts.
- `.claude/memory/decisions.md` stores persistent project context for Claude sessions.
